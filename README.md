...
2. **生态瞬时研究**：研究心理与行为如何在日常情境中动态变化、测量或适应性响应，覆盖 EMA/ESM、密集纵向、适应性干预、数字表型、被动感知、动态预测与日常多模态测量。
...
最终报告固定使用三个标准标签：`心脑轴`、`生态瞬时研究`、`心理微干预`。主题标签由 DeepSeek 语义筛选定义；本地规则只在调用模型前完成候选预筛。每篇入选论文包含中文标题、中文摘要、关键词、作者、期刊、IF、JCR 分区、发表时间和原文链接；跨主线或高优先级论文会进入“重点推荐”板块。
